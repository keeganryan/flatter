...
const std::string ElementaryRRZ::impl_name() {return "ElementaryRRZ";}

ElementaryRRZ::ElementaryRRZ(const Matrix& C, const Matrix& A, const Matrix& B,
                       bool accumulate_c,
...
ElementaryRRZ::~ElementaryRRZ() {
    unconfigure();
}

void ElementaryRRZ::unconfigure() {
...
void ElementaryRRZ::configure(const Matrix& C, const Matrix& A, const Matrix& B,
                           bool accumulate_c,
                           const ComputationContext& cc) {
...
void ElementaryRRZ::gemm_xx(unsigned int adr, unsigned int adc, unsigned int bdr, unsigned int bdc) {
    ...
    for (unsigned int i = 0; i < m; i += 1) {
        for (unsigned int j = 0; j < n; j += 1) {
            ...
            for (unsigned int l = 0; l < k; l += 1) {
                mpfr_mul_z(prod, A[i*adr + l*adc], B[l*bdr + j*bdc], rnd);
